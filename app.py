...
def page_player_manager():
    ...
    with c1:
        sel_list = players["name"].dropna().astype(str).sort_values().tolist() if not players.empty else []
        sel = st.selectbox("Select player", sel_list, index=0 if sel_list else None, key="pm_select")
        ...
        # Use the existing photo_url if any; treat as plain string
        photo_url = (current.get("photo_url") or "") if current is not None else ""

        # Robust uploader: JPG/PNG/HEIC -> PNG bytes -> upload -> public URL
        up = st.file_uploader("Avatar (JPG/PNG/HEIC)", type=["jpg", "jpeg", "png", "heic", "HEIC"], key="pm_avatar")
        if up is not None:
            img = _png_from_uploaded_file(up)  # uses your helper
            if img is not None:
                img = _square_thumbnail(img, size=384)  # consistent square avatar
                buf = io.BytesIO()
                img.save(buf, format="PNG", optimize=True)
                key = f"{uuid.uuid4().hex}.png"

                try:
                    photo_url = _storage_upload_png(buf.getvalue(), key)  # bytes, not BytesIO
                    st.image(photo_url, width=160, caption="Uploaded")
                except Exception as e:
                    st.error(f"Upload failed: {e}")

        # Save button: safe for None values
        if st.button("Save player", key="pm_save"):
            payload = {
            ...
            try:
                if current is not None and "id" in current and pd.notna(current["id"]):
    ...
    with c2:
        if current is not None:
            st.caption("Current")
            cur_url = (current.get("photo_url") or "").strip()
            if cur_url:
                st.image(cur_url, width=140)
            else:
                st.markdown(
                    f"<div class='card' style='width:140px;height:140px;border-radius:14px;"
                    f"display:flex;align-items:center;justify-content:center;"
                    f"font-size:32px;font-weight:800'>{name_initials(current['name'])}</div>",
                    unsafe_allow_html=True
                )

...
@st.cache_resource
def _client():  return create_client(SUPABASE_URL, SUPABASE_ANON_KEY)
@st.cache_resource
def _service(): return create_client(SUPABASE_URL, SUPABASE_SERVICE_KEY)
...
# image helpers (Player Manager uses these)
...
def _storage_upload_png(bytes_data: bytes, key: str) -> str:
    ...
    if isinstance(pub, dict) and "publicUrl" in pub: return pub["publicUrl"]
    return str(pub)
...
# ─────────────────────────────────────────────────────────────────────────────
# NORMALIZATION (fixes “not 1-dimensional”, datatype mismatches, empty renders)
# ─────────────────────────────────────────────────────────────────────────────
def normalize_lineups(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty: return pd.DataFrame(columns=[
        "id","season","gw","match_id","team","name","player_name","is_gk","goals","assists","line","slot","photo_url"
    ])
    out = df.copy()
    ...
    # flexible truthiness for GK
    if "is_gk" in out.columns:
        out["is_gk"] = out["is_gk"].astype(str).str.lower().isin(["1","true","t","yes","y"])
    else:
        out["is_gk"] = False
    out["team"]     = out.get("team").astype(str)
...
def normalize_matches(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in ["team_a","team_b","motm_name","formation_a","formation_b","id"]:
...
def team_pitch_svg(raw_rows: pd.DataFrame, formation: str, motm_name: Optional[str], left_side: bool) -> str:
    ...
    pitch.append("<defs><linearGradient id='g' x1='0' y1='0' x2='0' y2='1'>"
    ...
    if rows is not None and not rows.empty:
        ...
        for li, cnt in enumerate(parts, start=1):
            ...
            for s in range(1, cnt+1):
                r = rows[(rows["line"]==li) & (rows["slot"]==s)]
                if r.empty: continue
                rr = r.iloc[0]
                x = side_x0 + xgap * s
                y = cur_y
                pitch += _player_node(
                    x, y, rr["name"], int(rr["goals"]), int(rr["assists"]),
                    (motm_name or "").strip().lower() == rr["name"].strip().lower(),
...
def compute_player_ratings(fact: pd.DataFrame, name: str, min_gp_for_ratings=3):
    ...
    impact = int((35 + impact_raw*120)); impact = max(35, min(95, impact))
    return {"Finishing": fin, "Playmaking": ply, "Impact": impact, "Overall": int(round(0.4*fin + 0.4*ply + 0.2*impact))}
...
def teammates_duos_for(fact: pd.DataFrame, player: str, min_games=3, top_n=5) -> pd.DataFrame:
    ...
    return grp.reset_index().rename(columns={"name":"Teammate"})[["Teammate","GP","Win%"]]
...
def nemesis_for(fact: pd.DataFrame, player: str, min_games=3, top_n=5) -> pd.DataFrame:
    ...
    return grp.reset_index().rename(columns={"name":"Opponent"})[["Opponent","GP","Win%"]]
...
def header(key_prefix: str = "hdr"):
    ...
    with c2:
        ...
        if not st.session_state["is_admin"]:
            ...
            if st.button("Login", key=f"{key_prefix}_login"):
                if (pw or "") == ADMIN_PASSWORD:
                    st.session_state["is_admin"] = True
                    st.success("Admin mode on."); st.rerun()
                else:
    ...
    with c3:
        if st.session_state.get("is_admin"):
            if st.button("Logout", key=f"{key_prefix}_logout"):
                st.session_state["is_admin"] = False
                st.rerun()

# ─────────────────────────────────────────────────────────────────────────────
# LINEUP TEAM EDITOR (simple select-per-slot, mobile-friendly)
# ─────────────────────────────────────────────────────────────────────────────
def _team_names_from(df_team: pd.DataFrame) -> List[str]:
    if df_team is None or df_team.empty: return []
    return (
        df_team.get("name").fillna(df_team.get("player_name")).dropna().astype(str)
        .map(normalize_name).unique().tolist()
    )

def team_quick_assign(team_label: str, formation: str, df_team: pd.DataFrame,
                      team_player_names: List[str], keypref: str):
    cur = {}
    if df_team is not None and not df_team.empty:
        for _, r in df_team.iterrows():
            ...
            if li > 0 and s > 0 and nm: cur[(li, s)] = nm
    ...
    with c1:
        if st.button("Auto-fill", key=f"{keypref}_auto"): clicked["auto"] = True
    with c2:
        if st.button("Reset team", key=f"{keypref}_reset"): clicked["reset"] = True
    with c3:
        if st.button("Save team", type="primary", key=f"{keypref}_save"): clicked["save"] = True
    return grid, clicked
...
def save_team_grid(match_id, season, gw, team_label, formation, grid, df_team):
    ...
    if df_team is not None and not df_team.empty:
        for _, r in df_team.iterrows():
            ...
            if nm and nm not in sample: sample[nm] = r
    rows = []
    for (li, s), nm in grid:
        if not nm: continue
    ...
    svc.table("matches").update({"formation_a" if team_label=="Non-bibs" else "formation_b": formation}).eq("id", str(match_id)).execute()
    return True
...
def page_matches():
    ...
    if st.session_state.get("is_admin"):
        ...
        a_names = _team_names_from(a_rows); b_names = _team_names_from(b_rows)
        colA, colB = st.columns(2)
        with colA:
            gridA, clickA = team_quick_assign("Non-bibs", fa, a_rows, a_names, keypref=f"A_{mid}")
            if clickA["auto"]:  gridA = apply_autofill(gridA, a_names, fa); st.rerun()
            if clickA["reset"]: _service().table("lineups").delete().eq("match_id", str(mid)).eq("team","Non-bibs").execute(); st.rerun()
            if clickA["save"]:
        ...
        with colB:
            ...
            if clickB["auto"]:  gridB = apply_autofill(gridB, b_names, fb); st.rerun()
            if clickB["reset"]: _service().table("lineups").delete().eq("match_id", str(mid)).eq("team","Bibs").execute(); st.rerun()
            if clickB["save"]:
                if save_team_grid(mid, m["season"], m["gw"], "Bibs", fb, gridB, b_rows):
                    st.success("Bibs saved."); refresh_all(); st.rerun()

        # Quick goals/assists edit
        st.markdown("### Edit goals / assists")
        def _edit_team(df_team: pd.DataFrame, label: str):
            if df_team.empty: st.info(f"No {label} lineup yet."); return
            temp = df_team[["id","name","goals","assists"]].copy()
            ...
            with st.form(f"ga_{label}_{mid}"):
                ...
                g = st.number_input("Goals",   0, 50, int(temp[temp["Player"]==p]["G"].iloc[0]), key=f"ga_g_{label}_{mid}")
                a = st.number_input("Assists", 0, 50, int(temp[temp["Player"]==p]["A"].iloc[0]), key=f"ga_a_{label}_{mid}")
...
def page_players():
    ...
    lineups = normalize_lineups(load_table("lineups"))
    fact = build_fact(players, matches, lineups)
    if players.empty: st.info("No players."); return
    ...
    with c1:
        if p.get("photo_url"): st.image(p["photo_url"], width=220)
        else:
            st.markdown(
                ...
                f"<div style='font-size:48px;font-weight:800'>{name_initials(sel)}</div></div>", unsafe_allow_html=True
            )
        if str(p.get("notes") or "").strip(): st.caption(p["notes"])

    with c2:
        mine = fact[fact["name"]==sel]
        if mine.empty: st.info("No appearances yet."); return
    ...
    st.dataframe(duos, use_container_width=True, hide_index=True) if not duos.empty else st.info("No duo data yet.")
    ...
    st.dataframe(nem, use_container_width=True, hide_index=True) if not nem.empty else st.info("No nemesis data yet.")
...
def page_awards():
    ...
    if matches.empty: st.info("No matches.")
    else:
        mm = matches[['season','gw','motm_name']].dropna().sort_values(["season","gw"]).rename(columns={"motm_name":"MOTM"})
        st.dataframe(mm, use_container_width=True, hide_index=True)

    st.markdown("### Player of the Month")
    if awards.empty: st.info("No awards recorded.")
    else:
        potm = awards[awards["type"]=="POTM"][["season","month","player_name","notes"]].rename(columns={"player_name":"POTM"}).sort_values(["season","month"])
...
def page_player_manager():
    ...
    with left:
        ...
        with colA:
            if st.button("Remove photo", key="pm_remove_photo"): photo_url = ""
        with colB:
            if st.button("Save player", type="primary", key="pm_save"):
                payload = {"name": (new_name or "").strip(), "notes": (notes or "").strip(), "photo_url": (photo_url or "").strip()}
                try:
    ...
    with right:
        st.caption("Preview")
        if (photo_url or ""): st.image(photo_url, width=140)
        else:
            initials = name_initials((new_name or (sel if sel != "➕ Add new player" else "")))
            st.markdown(
                f"<div class='card' style='width:140px;height:140px;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:32px;font-weight:800'>{initials}</div>",
                unsafe_allow_html=True,
            )
        st.write("")
